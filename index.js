...
var emailSubscriptions = mysqlTable("emailSubscriptions", {
  id: int("id").autoincrement().primaryKey(),
  email: varchar("email", { length: 320 }).notNull().unique(),
  name: varchar("name", { length: 255 }),
  interest: varchar("interest", { length: 255 }),
  // e.g., "research", "implementation", "collaboration"
...
async function subscribeEmail(data) {
  const db = await getDb();
  if (!db) {
    throw new Error("Database not available");
  }
  try {
    await db.insert(emailSubscriptions).values(data).onDuplicateKeyUpdate({
      set: {
        name: data.name,
...
async function getSubscribedEmails() {
  ...
  try {
    return await db.select().from(emailSubscriptions).where(eq(emailSubscriptions.subscribed, 1));
  } catch (error) {
    console.error("[Database] Failed to get subscribed emails:", error);
    throw error;
  }
}
  ...
  contact: router({
    submit: publicProcedure.input(
      ...
      try {
        ...
        await notifyOwner({
          title: "New Contact Message from autoRA Website",
